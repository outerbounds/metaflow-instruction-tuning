...
class HuggingFaceLora:
    ...
    macro_batch_size = Parameter("macro-batch-size", help="macro batch size", default=128)
    visible_devices = Parameter("devices", help="visible devices", default=N_GPU, type=str)
    ...
    micro_batch_size = Parameter("micro-batch-szie", help="micro batch size", default=4)
    model_save_directory = Parameter("output-directory", help="model save directory", default="./lora-alpaca")
    master_port = Parameter("master-port", help="master port", default=1234)
    lora_r = Parameter("r", help="lora r", default=2)
    lora_target_modules = Parameter("target-modules", help="target lora modules you want to fine tune", default="[q_proj,v_proj]")
    ...
    def upload_to_huggingface(self):
        ...
        for checkpoint_folder in glob(self.model_save_directory + "/*/"):
            ...
            for ff in [f"{checkpoint_folder}/adapter_config.json",
